...
# The setting that actually delivers the restart the comment above
# describes. Without AutoRestartTime the Gateway never writes the
# autorestart token, and IBC logs on every start:
#   'autorestart file not found: full authentication will be required'
# Every restart then needs a full interactive login -- which is the
# path that once sat pending for 11.5 hours between 'Login attempt: 1'
# and 'Login has completed'.
#
# The container runs with TZ unset, so this time is UTC. 01:00 UTC is
# 21:00 ET, an hour after the 20:00 ET extended-hours close and well
# clear of the 09:30-16:00 ET regular session. Change this and the
# RTH timers together, or a restart will land mid-session.
AutoRestartTime=01:00 AM
