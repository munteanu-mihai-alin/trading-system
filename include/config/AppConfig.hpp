...
struct AppConfig {
  ...
  //
  // Chronos-2 runs in Python, not C++. Once per trading day the
  // Chronos2ExecutionEngine:
  //   1. Dumps the last N daily closes per symbol to a CSV under
  //      chronos2_daily_closes_dir.
  //   2. Spawns `chronos2_python chronos2_forecast_script --history-csv
  //      <path> --output <path> --context-len N --model MODEL`.
