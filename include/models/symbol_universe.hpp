#pragma once
#include <string>
#include <utility>
#include <vector>

namespace hft {

inline const std::vector<std::pair<std::string, std::string>>
    kLongGoalSymbolCompanyList = {{"NEX.FR", "Nexans France"},
                                  {"TTE.FR", "Total Energies"},
                                  {"BY6.DE", "BYD"},
                                  {"SAF.FR", "Safran"},
                                  {"ASML", "ASML"},
                                  {"ASML.AS", "ASML"},
                                  {"ASML.NL", "ASML"},
                                  {"ENI.IT", "Eni"},
                                  {"NOKIA.FI", "Nokia"},
                                  {"KOG.NO", "Kongsberg"},
                                  {"5711.T", "Mitsubishi Materials"},
                                  {"TYO:5711", "Mitsubishi Materials"},
                                  {"4043.T", "Tokuyama"},
                                  {"TYO:4043", "Tokuyama"},
                                  {"LEA.US", "Lear"},
                                  {"AWK.US", "American Water Works"},
                                  {"CEG.US", "Constellation Energy Corp"},
                                  {"VST.US", "Vistra Energy"},
                                  {"NIO.US", "Nio"},
                                  {"XPEV.US", "Xpeng"},
                                  {"INTC", "Intel"},
                                  {"INTC.US", "Intel"},
                                  {"OKLO.US", "Oklo Inc"},
                                  {"SNDK.US", "SanDisk"},
                                  {"MU.US", "Micron"},
                                  {"STX.US", "Seagate"},
                                  {"WDC.US", "Western Digital"},
                                  {"LMT.US", "Lockheed Martin"},
                                  {"HWM.US", "Howmet Aerospace"},
                                  {"RTX.US", "Raytheon Tech"},
                                  {"NOC.US", "Northrop Grumman"},
                                  {"GSM", "Ferroglobe"},
                                  {"WCH", "Wacker Chemie"},
                                  {"WCH.DE", "Wacker Chemie"},
                                  {"010060.KS", "OCI Holdings"},
                                  {"REC.OL", "REC Silicon"},
                                  {"4063.T", "Shin-Etsu Chemical"},
                                  {"3436.T", "SUMCO"},
                                  {"6488.TWO", "GlobalWafers"},
                                  {"WAF", "Siltronic"},
                                  {"WAF.DE", "Siltronic"},
                                  {"4186.T", "Tokyo Ohka Kogyo"},
                                  {"DD", "DuPont"},
                                  {"4901.T", "Fujifilm Holdings"},
                                  {"4005.T", "Sumitomo Chemical"},
                                  {"ENTG", "Entegris"},
                                  {"AI", "Air Liquide"},
                                  {"AIRP.PA", "Air Liquide"},
                                  {"LIN", "Linde"},
                                  {"APD", "Air Products"},
                                  {"MRK", "Merck KGaA"},
                                  {"MRK.DE", "Merck KGaA"},
                                  {"MKSI", "MKS Instruments"},
                                  {"AMAT", "Applied Materials"},
                                  {"LRCX", "Lam Research"},
                                  {"8035.T", "Tokyo Electron"},
                                  {"KLAC", "KLA"},
                                  {"ASM", "ASM International"},
                                  {"ASM.AS", "ASM International"},
                                  {"6857.T", "Advantest"},
                                  {"7735.T", "SCREEN Holdings"},
                                  {"SNPS", "Synopsys"},
                                  {"CDNS", "Cadence Design Systems"},
                                  {"SIE", "Siemens AG"},
                                  {"SIE.DE", "Siemens AG"},
                                  {"KEYS", "Keysight Technologies"},
                                  {"ARM", "Arm Holdings"},
                                  {"6533.TWO", "Andes Technology"},
                                  {"AMD", "AMD"},
                                  {"AAPL", "Apple"},
                                  {"QCOM", "Qualcomm"},
                                  {"2454.TW", "MediaTek"},
                                  {"NVDA", "NVIDIA"},
                                  {"005930.KS", "Samsung Electronics"},
                                  {"IBM", "IBM"},
                                  {"2330.TW", "TSMC"},
                                  {"TSM", "TSMC"},
                                  {"GFS", "GlobalFoundries"},
                                  {"2303.TW", "UMC"},
                                  {"UMC", "UMC"},
                                  {"0981.HK", "SMIC"},
                                  {"TSEM", "Tower Semiconductor"},
                                  {"5347.TWO", "VIS"},
                                  {"4062.T", "Ibiden"},
                                  {"3037.TW", "Unimicron"},
                                  {"ATS", "AT&S"},
                                  {"ATS.VI", "AT&S"},
                                  {"3189.TW", "Kinsus Interconnect"},
                                  {"009150.KS", "Samsung Electro-Mechanics"},
                                  {"2802.T", "Ajinomoto"},
                                  {"3711.TW", "ASE Technology Holding"},
                                  {"ASX", "ASE Technology Holding"},
                                  {"AMKR", "Amkor Technology"},
                                  {"600584.SS", "JCET"},
                                  {"6239.TW", "Powertech Technology"},
                                  {"8150.TW", "ChipMOS"},
                                  {"IMOS", "ChipMOS"},
                                  {"2449.TW", "KYEC"},
                                  {"DELL", "Dell Technologies"},
                                  {"HPQ", "HP Inc."},
                                  {"0992.HK", "Lenovo Group"},
                                  {"2357.TW", "ASUS"},
                                  {"2353.TW", "Acer"},
                                  {"SMCI", "Supermicro"},
                                  {"CSCO", "Cisco"},
                                  {"HPE", "HPE"}};

inline const std::vector<std::pair<std::string, std::string>>
    kSymbolCompanyList = {{"AAPL", "Apple"},
                          {"NVDA", "NVIDIA"},
                          {"AMD", "AMD"},
                          {"INTC", "Intel"},
                          {"MU", "Micron"},
                          {"QCOM", "Qualcomm"},
                          {"ARM", "Arm Holdings"},
                          {"ASML", "ASML"},
                          {"AMAT", "Applied Materials"},
                          {"LRCX", "Lam Research"},
                          {"KLAC", "KLA"},
                          {"SNPS", "Synopsys"},
                          {"CDNS", "Cadence Design Systems"},
                          {"MKSI", "MKS Instruments"},
                          {"ENTG", "Entegris"},
                          {"STX", "Seagate"},
                          {"WDC", "Western Digital"},
                          {"DELL", "Dell Technologies"},
                          {"HPQ", "HP Inc."},
                          {"SMCI", "Supermicro"},
                          {"CSCO", "Cisco"},
                          {"HPE", "HPE"},
                          {"IBM", "IBM"},
                          {"KEYS", "Keysight Technologies"},
                          {"TSM", "TSMC"},
                          {"GFS", "GlobalFoundries"},
                          {"UMC", "UMC"},
                          {"TSEM", "Tower Semiconductor"},
                          {"ASX", "ASE Technology Holding"},
                          {"AMKR", "Amkor Technology"},
                          {"IMOS", "ChipMOS"},
                          {"LEA", "Lear"},
                          {"AWK", "American Water Works"},
                          {"CEG", "Constellation Energy Corp"},
                          {"VST", "Vistra Energy"},
                          {"NIO", "Nio"},
                          {"XPEV", "Xpeng"},
                          {"OKLO", "Oklo Inc"},
                          {"SNDK", "SanDisk"},
                          {"LMT", "Lockheed Martin"},
                          {"HWM", "Howmet Aerospace"},
                          {"RTX", "RTX"},
                          {"NOC", "Northrop Grumman"},
                          {"GSM", "Ferroglobe"},
                          {"DD", "DuPont"},
                          {"LIN", "Linde"},
                          {"APD", "Air Products"},
                          {"TTE", "TotalEnergies ADR"},
                          {"NOK", "Nokia ADR"}};

// Loads a (symbol, company) list from a text file. Format: one symbol
// per line, optionally followed by ",company name". Lines that are
// empty or start with '#' are skipped. Trailing whitespace is stripped.
//
// Returns the parsed list. If the file is missing or unreadable, returns
// an empty vector; callers (typically LiveExecutionEngine::initialize_universe
// or main.cpp) decide whether that triggers a fallback to the hard-coded
// kSymbolCompanyList above.
//
// Implemented in src/lib/SymbolUniverse.cpp to keep this header
// dependency-free.
std::vector<std::pair<std::string, std::string>> load_symbol_universe_from_file(
    const std::string& path);

// Returns the IBKR `primaryExchange` override for a symbol, or empty if
// SMART routing alone is enough. Populated from
// agent/ibkr_symbol_audit.md (the live output of
// scripts/ibkr_symbol_contract_probe.py against the user's paper
// account) - any symbol whose reqContractDetails returns >1 contract or
// the wrong listing needs an explicit override. PSTG was long cited as
// the motivating example; it turned out to be delisted rather than
// ambiguous (see primary_exchange_override_table in SymbolUniverse.cpp)
// and has been removed from the universe. See agent/ibkr_client_audit.md
// #1 and #9 for the audit context.
//
// Thread-safe: returns a copy of an interned string; the underlying
// table is built once at static-init time.
[[nodiscard]] std::string primary_exchange_for(const std::string& symbol);

}  // namespace hft
