 ...
 public:
  ...
  bool cancel(int id) {
    for (auto* side : {&bids_, &asks_}) {
      for (auto& lvl : *side) {
        const auto it =
            std::find_if(lvl.queue.begin(), lvl.queue.end(),
  ...
  [[nodiscard]] double queue_ahead_at_level(double price, int my_id,
                                            bool is_buy) const {
  ...
  MatchResult match_at_price(double watch_price, int my_id) {
    ...
    while (!bids_.empty() && !asks_.empty() &&
           bids_.front().price >= asks_.front().price) {
      auto& bl = bids_.front();
      auto& al = asks_.front();
      if (bl.queue.empty()) {
      ...
      if (px == watch_price)
        out.traded_at_price += qty;
      if (b.id == my_id && b.is_mine)
        out.my_filled_qty += qty;
      if (a.id == my_id && a.is_mine)
        out.my_filled_qty += qty;
