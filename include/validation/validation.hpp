...
class ValidationMetrics {
 ...
 public:
  ...
  [[nodiscard]] double rolling_error_mean() const {
    if (rolling_error_.empty())
      return 0.0;
    double s = 0.0;
    for (double x : rolling_error_)
      s += x;
  ...
  [[nodiscard]] double ks_statistic() const {
    if (predicted_.empty() || realized_.empty())
      return 0.0;
    ...
    for (double x : grid) {
      while (ip < p.size() && p[ip] <= x)
        ++ip;
      while (ir < r.size() && r[ir] <= x)
        ++ir;
      const double cdf_p =
          static_cast<double>(ip) / static_cast<double>(p.size());
      const double cdf_r =
          static_cast<double>(ir) / static_cast<double>(r.size());
