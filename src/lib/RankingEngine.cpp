...
void RankingEngine::initialize(int n_stocks) {
    ...
    const int count = std::min<int>(n_stocks, static_cast<int>(kSymbolCompanyList.size()));
...
void RankingEngine::step(int t) {
    ...
    for (int i = 0; i < 4; ++i) simulator_.external_flow(order_book_);
    ...
    for (auto& s : portfolio.items) {
        ...
        for (int i = 0; i < 8; ++i) {
            ...
            const double queue_ahead = order_book_.queue_ahead_at_level(L, my_id, true);

            auto match = order_book_.match_at_price(L, my_id);
            const double p_pred = fill_model_.compute(match.traded_at_price, queue_ahead, dist);
    ...
    for (std::size_t i = 0; i < portfolio.items.size(); ++i) {
        ...
        portfolio.items[i].shadow_active = static_cast<int>(i) >= live_top_k_ &&
