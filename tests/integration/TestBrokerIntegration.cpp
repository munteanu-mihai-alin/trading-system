...
HFT_TEST(test_ranking_engine_initialize_resets_existing_portfolio) {
  ...
  hft::test::require(
      engine.portfolio.items[0].symbol == hft::kSymbolCompanyList[0].first,
...
HFT_TEST(test_ranking_engine_uses_symbol_universe_and_shadow_slots) {
  ...
  hft::test::require(
      engine.portfolio.items[0].company == hft::kSymbolCompanyList[0].second,
  ...
  hft::test::require(active_count == 3,
                     "three live/paper slots should be active");
  hft::test::require(shadow_count == 10,
                     "ten shadow slots should be marked locally");
}
...
HFT_TEST(test_ranking_engine_tracks_shadow_locally_only_after_multiple_steps) {
  ...
  for (const auto& s : engine.portfolio.items) {
    if (s.real.trades > 0)
      real_trade_names++;
    if (s.shadow.trades > 0)
      shadow_trade_names++;
  }

  hft::test::require(real_trade_names >= 0,
                     "real trade scan path should be valid");
  hft::test::require(shadow_trade_names >= 0,
                     "shadow trade scan path should be valid");
}

HFT_TEST(
    test_live_execution_engine_paper_mode_keeps_shadow_local_and_places_only_live_slots) {
  AppConfig cfg;
  cfg.mode = BrokerMode::Paper;
  cfg.top_k = 3;
  ...
  hft::test::require(
      static_cast<int>(raw->placed.size()) == 3,
