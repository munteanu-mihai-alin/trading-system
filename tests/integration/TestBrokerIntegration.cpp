...
HFT_TEST(test_ranking_engine_initialize_resets_existing_portfolio) {
    ...
    hft::test::require(engine.portfolio.items[0].symbol == hft::kSymbolCompanyList[0].first,
                       "initialize should rebuild symbols from universe");
}
...
HFT_TEST(test_ranking_engine_uses_symbol_universe_and_shadow_slots) {
    ...
    hft::test::require(engine.portfolio.items.size() == 20,
                       "engine should initialize requested count from universe");
    hft::test::require(engine.portfolio.items[0].symbol == hft::kSymbolCompanyList[0].first,
                       "first symbol should come from universe");
    hft::test::require(engine.portfolio.items[0].company == hft::kSymbolCompanyList[0].second,
                       "first company should come from universe");
...
HFT_TEST(test_live_execution_engine_paper_mode_keeps_shadow_local_and_places_only_live_slots) {
    ...
    hft::test::require(static_cast<int>(raw->placed.size()) == 3,
                       "paper/live routing should still place only three live slots");
