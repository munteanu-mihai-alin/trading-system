...
HFT_TEST(test_paper_broker_update_queue_drains_to_empty) {
    ...
    hft::test::require(!broker.poll_update(u), "queue should be empty after draining updates");
}
...
HFT_TEST(test_ranking_engine_validation_alarm_query_path) {
    ...
    hft::test::require(alarm == false || alarm == true, "alarm query path should be reachable");
}
...
HFT_TEST(test_paper_broker_disconnect_after_multiple_operations) {
    ...
    broker.disconnect();
    hft::test::require(!broker.is_connected(), "paper broker should report disconnected");
}
...
HFT_TEST(test_ranking_engine_initialize_resets_existing_portfolio) {
    ...
    hft::test::require(engine.portfolio.items.size() == 2, "initialize should reset portfolio size");
    hft::test::require(engine.portfolio.items[0].symbol == hft::kSymbolCompanyList[0].first, "initialize should rebuild symbols from universe");
}
...
HFT_TEST(test_ranking_engine_marks_only_top_k_active_after_step) {
    ...
    hft::test::require(active_count == 1, "exactly top_k stocks should be active after step");
}

HFT_TEST(test_ranking_engine_records_validation_and_cycles_after_multiple_steps) {
    ...
    hft::test::require(engine.validation.size() > 0, "validation samples should accumulate");
    hft::test::require(!engine.cycle_samples.empty(), "cycle samples should accumulate");
    hft::test::require(engine.validation.calibration_error() >= 0.0, "calibration error query path should be valid");
    hft::test::require(engine.validation.ks_statistic() >= 0.0, "ks query path should be valid");
}
...
HFT_TEST(test_ranking_engine_shadow_and_real_pnl_queries) {
    ...
    hft::test::require(real_pnl_sum == real_pnl_sum, "real pnl sum should be finite");
    hft::test::require(shadow_pnl_sum == shadow_pnl_sum, "shadow pnl sum should be finite");
}

HFT_TEST(test_ibkr_client_stub_connect_disconnect_and_status_queries) {
    IBKRClient client;
    hft::test::require(client.connect("127.0.0.1", 4002, 17), "stub connect should succeed");
    hft::test::require(client.is_connected(), "client should report connected after connect");
    client.cancel_order(999);
    client.disconnect();
    hft::test::require(!client.is_connected(), "client should report disconnected after disconnect");
}
...
HFT_TEST(test_ibkr_client_stub_multiple_order_lifecycle_entries) {
    ...
    hft::test::require(client.lifecycle().has(101), "first lifecycle entry should exist");
    hft::test::require(client.lifecycle().has(102), "second lifecycle entry should exist");
    hft::test::require(client.lifecycle().get(101)->symbol == "AAPL", "first symbol should match");
    hft::test::require(client.lifecycle().get(102)->symbol == "MSFT", "second symbol should match");
...
HFT_TEST(test_live_execution_engine_multiple_steps_with_paper_broker) {
    ...
    hft::test::require(static_cast<int>(raw->placed.size()) == 12, "each step should place top_k orders");
    eng.stop();
}
...
HFT_TEST(test_ranking_engine_uses_symbol_universe_and_shadow_slots) {
    RankingEngine engine(3, "tmp_symbol_shadow.csv");
    engine.initialize(20);
    hft::test::require(engine.portfolio.items.size() == 20, "engine should initialize requested count from universe");
    hft::test::require(engine.portfolio.items[0].symbol == hft::kSymbolCompanyList[0].first, "first symbol should come from universe");
    hft::test::require(engine.portfolio.items[0].company == hft::kSymbolCompanyList[0].second, "first company should come from universe");
    ...
    for (const auto& s : engine.portfolio.items) {
        if (s.active) active_count++;
        if (s.shadow_active) shadow_count++;
    }
    hft::test::require(active_count == 3, "three live/paper slots should be active");
    hft::test::require(shadow_count == 10, "ten shadow slots should be marked locally");
}

HFT_TEST(test_ranking_engine_tracks_shadow_locally_only_after_multiple_steps) {
    RankingEngine engine(3, "tmp_symbol_shadow_trades.csv");
    engine.initialize(20);
    ...
    int real_trade_names = 0;
    int shadow_trade_names = 0;
    for (const auto& s : engine.portfolio.items) {
        if (s.real.trades > 0) real_trade_names++;
        if (s.shadow.trades > 0) shadow_trade_names++;
    }

    hft::test::require(real_trade_names >= 0, "real trade scan path should be valid");
    hft::test::require(shadow_trade_names >= 0, "shadow trade scan path should be valid");
}

HFT_TEST(test_live_execution_engine_paper_mode_keeps_shadow_local_and_places_only_live_slots) {
    AppConfig cfg;
    cfg.mode = BrokerMode::Paper;
    cfg.top_k = 3;
    ...
    hft::test::require(eng.start(), "paper engine should start");
    eng.initialize_universe(20);
    eng.step(0);

    int shadow_marked = 0;
    for (const auto& s : eng.ranking.portfolio.items) {
        if (s.shadow_active) shadow_marked++;
    }

    hft::test::require(static_cast<int>(raw->placed.size()) == 3, "paper/live routing should still place only three live slots");
    hft::test::require(shadow_marked == 10, "shadow names should remain local-only and marked");
    eng.stop();
}
