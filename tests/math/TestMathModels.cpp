...
HFT_TEST(test_fill_increases_with_traded_volume) {
  ...
  hft::test::require(p2 > p1,
                     "more traded volume should increase fill probability");
}
...
HFT_TEST(test_microprice_between_bid_and_ask) {
  ...
  hft::test::require(m >= 100.0 && m <= 100.2,
                     "microprice must lie inside spread");
}
...
HFT_TEST(test_fill_probability_increases_when_distance_grows_cross_branch) {
  ...
  hft::test::require(far_p > near_p,
                     "crossing component should rise with distance parameter "
                     "in current model");
}
...
HFT_TEST(test_latency_summary_empty_and_nonempty) {
  ...
  hft::test::require_close(empty.avg, 0.0, 1e-12,
                           "empty summary avg should be zero");
...
HFT_TEST(test_live_trading_config_mode_names) {
  ...
  hft::test::require(
      LiveTradingConfig::from_app(paper_cfg).mode_name() == "paper",
...
HFT_TEST(test_l2_book_best_prices_default_and_set) {
  L2Book book;
  hft::test::require_close(book.best_bid(), 0.0, 1e-12,
                           "default best bid should be zero");
  hft::test::require_close(book.best_ask(), 0.0, 1e-12,
                           "default best ask should be zero");
  book.bids[0] = {101.0, 10.0};
  book.asks[0] = {101.5, 12.0};
  hft::test::require_close(book.best_bid(), 101.0, 1e-12,
                           "best bid should reflect first level");
  hft::test::require_close(book.best_ask(), 101.5, 1e-12,
                           "best ask should reflect first level");
}
...
HFT_TEST(test_fill_model_clamps_extreme_inputs) {
  ...
  hft::test::require(low >= 0.0 && low <= 1.0,
                     "fill model should clamp low extreme");
  hft::test::require(high >= 0.0 && high <= 1.0,
                     "fill model should clamp high extreme");
}
...
HFT_TEST(test_queue_tracker_clamps_negative_queue_to_zero) {
  ...
  hft::test::require_close(s.queue_ahead, 0.0, 1e-12,
                           "queue ahead should clamp at zero");
}
...
HFT_TEST(test_l2_book_best_prices_after_partial_assignment) {
  ...
  hft::test::require_close(b.best_ask(), 0.0, 1e-12,
                           "empty ask side should stay zero");
}
...
HFT_TEST(test_stock_default_flags_and_fields) {
  ...
  hft::test::require_close(s.score, 0.0, 1e-12,
                           "stock score should default to zero");
}
