...
HFT_TEST(test_ranked_portfolio_orders_negative_and_positive_scores) {
    ...
    p.items = {a, b, c};
    p.rank();
    hft::test::require_close(p.items[0].score, 0.5, 1e-12, "highest positive score should be first");
...
HFT_TEST(test_latency_model_multiple_records_average) {
    ...
    m.record(3.0);
    m.record(5.0);
    hft::test::require_close(m.mean_latency(), 3.0, 1e-12, "mean latency should average multiple samples");
}
...
HFT_TEST(test_trade_stats_all_losses_zero_win_rate) {
    ...
    s.update(-1.0);
    s.update(-2.0);
    hft::test::require_close(s.win_rate(), 0.0, 1e-12, "all-loss trade stats should have zero win rate");
}

HFT_TEST(test_l2_book_best_prices_after_partial_assignment) {
    L2Book b;
    b.bids[0] = {99.5, 3.0};
    hft::test::require_close(b.best_bid(), 99.5, 1e-12, "best bid should reflect populated bid side");
