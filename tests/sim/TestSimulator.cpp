...
HFT_TEST(test_queue_ahead_reports_front_volume) {
  ...
  hft::test::require_close(q, 40.0, 1e-12,
                           "queue ahead should equal prior quantity at level");
}
...
HFT_TEST(test_match_reports_traded_volume_at_watch_price) {
  ...
  hft::test::require_close(
      res.traded_at_price, 100.0, 1e-12,
...
HFT_TEST(test_queue_ahead_zero_when_order_missing) {
  ...
  hft::test::require_close(q, 0.0, 1e-12,
                           "queue ahead should be zero for missing level");
}

HFT_TEST(test_my_order_state_realized_fill_false_then_true) {
  MyOrderState s;
  s.reset(1, 100.0, 50.0);
  hft::test::require(!s.realized_fill(),
                     "fresh order should not be realized fill");
  s.on_traded(50.0, 5.0);
  hft::test::require(s.realized_fill(),
                     "positive fill qty should mark realized fill");
}
...
HFT_TEST(test_match_reports_my_fill_when_front_order_is_mine) {
  ...
  hft::test::require_close(res.my_filled_qty, 25.0, 1e-12,
                           "my front order should fill");
}
...
HFT_TEST(test_queue_tracker_reset_overwrites_state) {
  ...
  hft::test::require_close(s.queue_ahead, 5.0, 1e-12,
                           "reset should update queue ahead");
  hft::test::require_close(s.filled_qty, 0.0, 1e-12,
                           "reset should zero filled quantity");
}
...
HFT_TEST(test_queue_tracker_zero_fill_keeps_realized_fill_false) {
  ...
  s.on_traded(1.0, 0.0);
  hft::test::require(!s.realized_fill(),
                     "zero fill quantity should keep realized_fill false");
}
...
HFT_TEST(test_orderbook_empty_match_returns_zeroes) {
  ...
  hft::test::require_close(res.traded_at_price, 0.0, 1e-12,
                           "empty order book should not trade");
  hft::test::require_close(res.my_filled_qty, 0.0, 1e-12,
                           "empty order book should not fill");
}

HFT_TEST(test_orderbook_cancel_missing_from_empty_book) {
  OrderBook ob;
  hft::test::require(!ob.cancel(1),
                     "empty order book should not cancel missing id");
}

HFT_TEST(test_orderbook_queue_ahead_on_ask_side) {
  OrderBook ob;
  ob.add(OBOrder{1, 101.0, 10.0, false, false});
  ob.add(OBOrder{2, 101.0, 15.0, false, true});
  const double q = ob.queue_ahead_at_level(101.0, 2, false);
  hft::test::require_close(
      q, 10.0, 1e-12, "ask-side queue ahead should count prior ask volume");
}

HFT_TEST(test_orderbook_match_clears_empty_front_levels) {
  OrderBook ob;
  ob.add(OBOrder{1, 100.0, 5.0, true, false});
  ob.add(OBOrder{2, 100.0, 5.0, false, false});
  auto res1 = ob.match_at_price(100.0, 999);
  hft::test::require_close(res1.traded_at_price, 5.0, 1e-12,
                           "first cross should trade");
  auto res2 = ob.match_at_price(100.0, 999);
  hft::test::require_close(res2.traded_at_price, 0.0, 1e-12,
                           "second cross should see cleared levels");
}

HFT_TEST(test_queue_tracker_accumulates_fill_qty_across_calls) {
  MyOrderState s;
  s.reset(1, 100.0, 20.0);
  s.on_traded(5.0, 2.0);
  s.on_traded(5.0, 3.0);
  hft::test::require_close(s.filled_qty, 5.0, 1e-12,
                           "filled quantity should accumulate");
  hft::test::require(s.realized_fill(),
                     "accumulated fill should mark realized fill");
}
