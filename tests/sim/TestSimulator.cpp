...
HFT_TEST(test_orderbook_cancel_missing_from_empty_book) {
    OrderBook ob;
    hft::test::require(!ob.cancel(1), "empty order book should not cancel missing id");
}
...
HFT_TEST(test_orderbook_queue_ahead_on_ask_side) {
    ...
    hft::test::require_close(q, 10.0, 1e-12, "ask-side queue ahead should count prior ask volume");
}
...
HFT_TEST(test_orderbook_match_clears_empty_front_levels) {
    ...
    hft::test::require_close(res1.traded_at_price, 5.0, 1e-12, "first cross should trade");
    auto res2 = ob.match_at_price(100.0, 999);
    hft::test::require_close(res2.traded_at_price, 0.0, 1e-12, "second cross should see cleared levels");
}
...
HFT_TEST(test_queue_tracker_accumulates_fill_qty_across_calls) {
    ...
    s.on_traded(5.0, 2.0);
    s.on_traded(5.0, 3.0);
    hft::test::require_close(s.filled_qty, 5.0, 1e-12, "filled quantity should accumulate");
    hft::test::require(s.realized_fill(), "accumulated fill should mark realized fill");
}
