...
HFT_TEST(test_validation_calibration_bins_empty_counts_zero) {
    ...
    hft::test::require(total == 0, "empty validation bins should all have zero count");
}
...
HFT_TEST(test_validation_alarm_on_rolling_error_only) {
    ...
    hft::test::require(v.degradation_alarm(2.0, 0.5, 2.0), "rolling error threshold alone should trigger alarm");
}
